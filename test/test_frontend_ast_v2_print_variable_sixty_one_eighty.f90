program test_frontend_ast_v2_print_variable_sixty_one_eighty
    ...
    integer, parameter :: item_counts(3) = [61, 70, 80]
    ...
    do item_count = 1, size(item_counts)
        if (.not. ffc_lower_frontend_ast_v2_from_sx(positive_sx(item_counts(item_count)), body, message)) then
            ...
            error stop '61/70/80 stored-variable PRINT route was rejected'
        end if
    ...
    write (*, '(a)') 'frontend AST v2 variable 61/70/80-item PRINT checks: ok'
...
contains
    ...
    function positive_sx(item_count) result(value)
        ...
        value = '(program-unit-v2 (root (program-root (name main) (span (source-span '// &
            '(file main.f90) (start-byte 0) (end-byte 100) (source-hash print-variable-61-80))))) '// &
            ...
            '(source-hash print-variable-61-80))))) (variable-count 1) '// &
            '(variable (variable-declaration (type-spec integer) (name x) (span (source-span '// &
            '(file main.f90) (start-byte 10) (end-byte 20) (source-hash print-variable-61-80))))) '// &
            ...
            '(source-hash print-variable-61-80))))) '// &
            '(assignment (assignment-stmt (variable x) (expression (assignment-expression '// &
...
end program test_frontend_ast_v2_print_variable_sixty_one_eighty
