program test_frontend_ast_v2_print_variable_expression
    ...
    call assert_true(lower_positive(), 'source-backed assignment/PRINT sequence was rejected')
    ...
    call assert_true(body%instructions(1)%literal_value == 23 .and. &
        body%instructions(4)%literal_value == 1, 'constant transport changed')
    ...
    write (*, '(a)') 'frontend AST v2 variable expression PRINT checks: ok'

contains

    logical function lower_positive() result(ok)
        ok = ffc_lower_frontend_ast_v2_from_sx(positive_sx(), body, message)
    ...
    function positive_sx() result(value)
        ...
        value = '(program-unit-v2 (root (program-root (name main) (span (source-span '// &
            '(file main.f90) (start-byte 0) (end-byte 70) (source-hash print-variable-expression))))) '// &
            ...
            '(source-hash print-variable-expression))))) (variable-count 1) '// &
            '(variable (variable-declaration (type-spec integer) (name x) (span (source-span '// &
            '(file main.f90) (start-byte 10) (end-byte 20) (source-hash print-variable-expression))))) '// &
            ...
            '(kind integer-literal) (operator ) (left-operand 23) (right-operand ))) '// &
            '(span (source-span (file main.f90) (start-byte 25) (end-byte 31) '// &
            '(source-hash print-variable-expression))))) '// &
            '(assignment (assignment-stmt (variable x) (expression (assignment-expression '// &
...
end program test_frontend_ast_v2_print_variable_expression
