program test_frontend_ast_v2_init_xminus_range
    ...
    use ffc_mir, only: mir_function_body_t, mir_validate_function_body, opcode_const, &
        opcode_load, opcode_output, opcode_return, opcode_store, opcode_sub, &
        value_kind_integer
    implicit none

    type(mir_function_body_t) :: body
    character(len=:), allocatable :: message
    integer(int32) :: initializers(2), subtrahends(2)
    integer :: initializer_index, subtrahend_index

    initializers = [-42_int32, 42_int32]
    subtrahends = [2_int32, 10_int32]
    do initializer_index = 1, size(initializers)
        do subtrahend_index = 1, size(subtrahends)
            call check_case(initializers(initializer_index), &
                subtrahends(subtrahend_index))
        end do
    end do

    call assert_rejected(witness('42', '0', '–'), 'zero subtrahend was accepted')
...
contains
    ...
    subroutine check_case(initializer, subtrahend)
        integer(int32), intent(in) :: initializer, subtrahend
        integer(int32) :: expected_literals(9), expected_opcodes(9), expected_results(9)
        integer :: instruction_index
        character(len=32) :: initializer_literal, subtrahend_literal

        write (initializer_literal, '(i0)') initializer
        write (subtrahend_literal, '(i0)') subtrahend
        call assert_true(ffc_lower_frontend_ast_v2_from_sx(&
            witness(trim(initializer_literal), trim(subtrahend_literal), '–'), &
            body, message), &
            'initialized subtraction sequence was rejected')
        ...
        expected_literals(4) = subtrahend
    ...
    function witness(initializer, subtrahend, operator) result(value)
        character(len=*), intent(in) :: initializer, subtrahend, operator
        ...
        value = '(program-unit-v2 (root (program-root (name main) '// &
            ...
            '(source-hash init-subtraction))))) '// &
            '(declaration-count 1) (declaration (program-declaration '// &
            '(declaration-kind program) '// &
            '(name main) (span (source-span (file main.f90) '// &
            '(start-byte 0) (end-byte 70) '// &
            '(source-hash init-subtraction))))) '// &
            '(variable-count 1) (variable (variable-declaration '// &
            '(type-spec integer) (name x) (span (source-span '// &
            '(file main.f90) (start-byte 10) (end-byte 20) '// &
            '(source-hash init-subtraction))))) '// &
            ...
            '(left-operand INITIAL) '// &
            '(right-operand ))) (span (source-span (file main.f90) '// &
            '(start-byte 25) (end-byte 31) (source-hash init-subtraction))))) '// &
            '(assignment (assignment-stmt (variable x) '// &
            '(expression (assignment-expression (kind binary-expression) '// &
            '(operator OPERATOR) (left-operand x) (right-operand SUBTRAHEND))) '// &
            '(span (source-span (file main.f90) '// &
            '(start-byte 32) (end-byte 42) '// &
            '(source-hash init-subtraction)))))) (print-stmt '// &
        ...
        value = replace_text(value, 'SUBTRAHEND', trim(subtrahend))
...
end program test_frontend_ast_v2_init_xminus_range
