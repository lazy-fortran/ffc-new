program test_frontend_ast_v2_initialized_variable_counter2_multiply
    use, intrinsic :: iso_fortran_env, only: int32
    use ffc_frontend_ast, only: ffc_lower_frontend_ast_v2_from_sx
    use ffc_mir, only: mir_function_body_t, mir_validate_function_body, opcode_const, opcode_load, opcode_mul, &
    ...
    if (.not. ffc_lower_frontend_ast_v2_from_sx(witness(), body, message)) then
        ...
        error stop 'raw counter_2 multiplication source was rejected'
    end if
    call assert_true(mir_validate_function_body(body, message), 'generated MIR is invalid')
    expected_opcodes = [opcode_const, opcode_store, opcode_load, opcode_const, opcode_mul, &
    ...
    call assert_true(body%instructions(4)%literal_value == 2_int32, 'multiplier literal changed')
    ...
    call assert_false(ffc_lower_frontend_ast_v2_from_sx(replace_text(witness(), &
        ...
        'wrong multiplication operator was accepted')
    call assert_false(ffc_lower_frontend_ast_v2_from_sx(replace_text(witness(), &
        '(name counter_2)', '(name counter_3)'), body, message), 'wrong variable name was accepted')
    call assert_false(ffc_lower_frontend_ast_v2_from_sx(replace_text(witness(), &
        '(right-operand 2)', '(right-operand 3)'), body, message), 'mutated multiplier was accepted')

    write (*, '(a)') 'frontend AST v2 raw counter_2 multiplication MIR checks: ok'
...
contains
    ...
    function witness() result(value)
        ...
        value = '(program-unit-v2 (root (program-root (name main) (span (source-span '// &
            '(file main.f90) (start-byte 0) (end-byte 100) (source-hash counter2-mul))))) '// &
            ...
            '(source-hash counter2-mul))))) (variable-count 1) (variable (variable-declaration '// &
            '(type-spec integer) (name counter_2) (span (source-span (file main.f90) '// &
            '(start-byte 10) (end-byte 25) (source-hash counter2-mul))))) (execution-part '// &
            ...
            '(start-byte 30) (end-byte 47) (source-hash counter2-mul))))) (assignment (assignment-stmt '// &
            ...
            '(start-byte 48) (end-byte 70) (source-hash counter2-mul)))))) (print-stmt '// &
...
end program test_frontend_ast_v2_initialized_variable_counter2_multiply
