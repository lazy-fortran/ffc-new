program test_frontend_ast_v2_print_variable_five_items
    ...
    integer(int32) :: expected_opcodes(17), expected_results(17)
    integer :: instruction_index

    expected_opcodes = [opcode_const, opcode_store, opcode_load, opcode_const, opcode_pow, &
        opcode_store, opcode_load, opcode_output, opcode_load, opcode_output, opcode_load, &
        opcode_output, opcode_load, opcode_output, opcode_load, opcode_output, opcode_return]
    expected_results = [0_int32, 1_int32, 2_int32, 3_int32, 4_int32, 4_int32, 6_int32, &
        6_int32, 7_int32, 7_int32, 8_int32, 8_int32, 9_int32, 9_int32, 10_int32, 10_int32, &
        10_int32]
    if (.not. ffc_lower_frontend_ast_v2_from_sx(positive_sx(), body, message)) then
        if (allocated(message)) write (*, '(a)') trim(message)
        error stop 'source-backed five-item PRINT sequence was rejected'
    end if
    call assert_true(mir_validate_function_body(body, message), 'generated MIR is invalid')
    call assert_true(body%function%instruction_count == 17_int32, 'instruction count changed')
    do instruction_index = 1, 17
    ...
    call assert_true(body%instructions(1)%literal_value == 3 .and. &
        body%instructions(4)%literal_value == 2, 'constant transport changed')
    call assert_storage(2)
    call assert_storage(3)
    call assert_storage(6)
    call assert_storage(7)
    call assert_storage(9)
    call assert_storage(11)
    call assert_storage(13)
    call assert_storage(15)
    write (*, '(a)') 'frontend AST v2 variable five-item PRINT checks: ok'
...
contains
    ...
    function positive_sx() result(value)
        ...
        value = '(program-unit-v2 (root (program-root (name main) (span (source-span '// &
            '(file main.f90) (start-byte 0) (end-byte 92) (source-hash print-variable-five-item))))) '// &
            '(declaration-count 1) (declaration (program-declaration (declaration-kind program) '// &
            '(name main) (span (source-span (file main.f90) (start-byte 0) (end-byte 92) '// &
            '(source-hash print-variable-five-item))))) (variable-count 1) '// &
            '(variable (variable-declaration (type-spec integer) (name x) (span (source-span '// &
            '(file main.f90) (start-byte 10) (end-byte 20) (source-hash print-variable-five-item))))) '// &
            ...
            '(source-hash print-variable-five-item))))) '// &
            '(assignment (assignment-stmt (variable x) (expression (assignment-expression '// &
            ...
            '(output-name x) (output-count 5) (output-kind-2 variable) (output-name-2 x) '// &
            '(output-rule-2 R901) (output-kind-3 variable) (output-name-3 x) (output-rule-3 R901) '// &
            '(output-kind-4 variable) (output-name-4 x) (output-rule-4 R901) '// &
            '(output-kind-5 variable) (output-name-5 x) (output-rule-5 R901) '// &
    ...
    subroutine assert_storage(instruction_index)
        ...
        call assert_true(allocated(body%instructions(instruction_index)%storage_key), &
            'storage key missing')
        call assert_equal(body%instructions(instruction_index)%storage_key, 'x', &
            'storage key changed')
...
end program test_frontend_ast_v2_print_variable_five_items
