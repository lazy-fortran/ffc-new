program test_frontend_ast_v2_initialized_variable_divide
    ...
    if (.not. ffc_lower_frontend_ast_v2_from_sx(witness(), body, message)) then
        ...
        error stop 'initialized variable-divide source was rejected'
    end if
    ...
    call assert_true(body%instructions(1)%literal_value == -42_int32, &
        'negative initializer literal changed')
    ...
    call assert_false(ffc_lower_frontend_ast_v2_from_sx(replace_text(witness(), &
        ...
        'mutated RHS was accepted')
    call assert_false(ffc_lower_frontend_ast_v2_from_sx(replace_text(witness(), &
        '(source-hash initialized-variable-divide)', '(source-hash mutated)'), body, message), &
        'mutated source identity was accepted')

    write (*, '(a)') 'frontend AST v2 initialized variable-divide MIR checks: ok'
...
contains
    ...
    function witness() result(value)
        ...
        value = '(program-unit-v2 (root (program-root (name main) (span (source-span '// &
            ...
            '(source-hash initialized-variable-divide))))) (declaration-count 1) '// &
            ...
            '(source-hash initialized-variable-divide))))) (variable-count 1) '// &
            '(variable (variable-declaration (type-spec integer) (name x) '// &
            '(span (source-span (file main.f90) (start-byte 10) (end-byte 20) '// &
            '(source-hash initialized-variable-divide))))) (execution-part '// &
            ...
            '(source-hash initialized-variable-divide))))) (assignment (assignment-stmt '// &
            ...
            '(source-hash initialized-variable-divide)))))) (print-stmt '// &
            ...
            '(output-page 248) (source-hash initialized-variable-divide))))'
...
end program test_frontend_ast_v2_initialized_variable_divide
