program test_frontend_ast_v2_init_xplus1_range
    ...
    values = [-100_int32, -42_int32, 0_int32, 42_int32, 2047_int32]
    do value_index = 1, size(values)
        call check_initializer(values(value_index))
    end do
    ...
    call assert_false(ffc_lower_frontend_ast_v2_from_sx(replace_text(witness('42'), &
        '(right-operand 1)', '(right-operand 2)'), body, message), 'wrong AST right was accepted')
    call assert_false(ffc_lower_frontend_ast_v2_from_sx(replace_text(witness('42'), &
        '(assignment-stmt (variable x)', '(assignment-stmt (variable y)'), body, message), &
        'wrong AST assignment name was accepted')
    write (*, '(a)') 'frontend AST v2 initialized x+1 MIR range checks: ok'
...
contains
    ...
    subroutine check_initializer(expected)
        ...
        call assert_true(body%instructions(1)%literal_value == expected .and. &
            body%instructions(4)%literal_value == 1, 'literal transport changed')
    ...
    function witness(literal) result(value)
        ...
        value = '(program-unit-v2 (root (program-root (name main) (span (source-span '// &
            ...
            '(right-operand 1))) (span (source-span (file main.f90) (start-byte 32) '// &
