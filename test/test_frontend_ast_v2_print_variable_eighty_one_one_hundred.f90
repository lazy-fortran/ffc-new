program test_frontend_ast_v2_print_variable_eighty_one_one_hundred
    ...
    integer, parameter :: item_counts(3) = [81, 90, 100]
    integer :: item_count, index

    do item_count = 1, size(item_counts)
        if (.not. ffc_lower_frontend_ast_v2_from_sx(positive_sx(item_counts(item_count)), body, message)) then
            if (allocated(message)) write (*, '(a)') trim(message)
            error stop '81/90/100 stored-variable PRINT route was rejected'
        end if
        call assert_true(mir_validate_function_body(body, message), 'generated MIR is invalid')
        call assert_true(body%function%instruction_count == 2 * item_counts(item_count) + 7, &
            'instruction count changed')
        do index = 7, 2 * item_counts(item_count) + 6, 2
    ...
    write (*, '(a)') 'frontend AST v2 variable 81/90/100-item PRINT checks: ok'
...
contains
    ...
    function positive_sx(item_count) result(value)
        ...
        value = '(program-unit-v2 (root (program-root (name main) (span (source-span '// &
            '(file main.f90) (start-byte 0) (end-byte 100) (source-hash print-variable-81-100))))) '// &
            ...
            '(source-hash print-variable-81-100))))) (variable-count 1) '// &
            '(variable (variable-declaration (type-spec integer) (name x) (span (source-span '// &
            '(file main.f90) (start-byte 10) (end-byte 20) (source-hash print-variable-81-100))))) '// &
            ...
            '(source-hash print-variable-81-100))))) '// &
            '(assignment (assignment-stmt (variable x) (expression (assignment-expression '// &
...
end program test_frontend_ast_v2_print_variable_eighty_one_one_hundred
