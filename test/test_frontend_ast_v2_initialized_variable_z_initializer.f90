program test_frontend_ast_v2_initialized_variable_z_initializer
    ...
    call assert_false(ffc_lower_frontend_ast_v2_from_sx(replace_text(witness('5'), &
        '(source-hash initialized-variable-z-initializer)', '(source-hash mutated)'), body, message), &
        'mutated source provenance was accepted')

    write (*, '(a)') 'frontend AST v2 initialized variable-z initializer checks: ok'
...
contains
    ...
    subroutine check_initializer(expected)
        ...
        if (.not. ffc_lower_frontend_ast_v2_from_sx(witness(trim(literal)), body, message)) then
            ...
            error stop 'variable-z initializer was rejected'
        end if
    ...
    subroutine assert_storage(index)
        ...
        call assert_equal(body%instructions(index)%storage_key, 'z', 'storage key changed')
    ...
    function witness(literal) result(value)
        ...
        value = '(program-unit-v2 (root (program-root (name main) (span (source-span '// &
            ...
            '(source-hash initialized-variable-z-initializer))))) (declaration-count 1) '// &
            ...
            '(source-hash initialized-variable-z-initializer))))) (variable-count 1) '// &
            '(variable (variable-declaration (type-spec integer) (name z) '// &
            '(span (source-span (file main.f90) (start-byte 10) (end-byte 20) '// &
            '(source-hash initialized-variable-z-initializer))))) (execution-part '// &
            '(assignment-sequence (assignment-count 1) (assignment (assignment-stmt '// &
            '(variable z) (expression (assignment-expression (kind integer-literal) '// &
            '(operator ) (left-operand LITERAL) (right-operand ))) (span (source-span '// &
            '(file main.f90) (start-byte 25) (end-byte 31) '// &
            '(source-hash initialized-variable-z-initializer)))))) (print-stmt '// &
            '(format-kind default-char-expr) (format-value *) (output-kind variable) '// &
            '(output-name z) (statement-rule R1212) (format-rule R1215) (output-rule R901) '// &
            ...
            '(source-hash initialized-variable-z-initializer))))'
...
end program test_frontend_ast_v2_initialized_variable_z_initializer
