program test_frontend_ast_v2_print_mul
    ...
    use ffc_mir, only: mir_function_body_t, mir_validate_function_body, opcode_const, opcode_load, &
        opcode_mul, opcode_output, opcode_return, opcode_store, value_kind_integer
    implicit none
    ...
    if (.not. ffc_lower_frontend_ast_v2_from_sx(positive_sx(), body, message)) then
        ...
        error stop 'source-backed multiplication PRINT sequence was rejected'
    end if
...
contains
    ...
    function positive_sx() result(value)
        ...
        value = '(program-unit-v2 (root (program-root (name main) (span (source-span '// &
            '(file main.f90) (start-byte 0) (end-byte 70) (source-hash print-variable-multiply-expression))))) '// &
            ...
            '(source-hash print-variable-multiply-expression))))) (variable-count 1) '// &
            '(variable (variable-declaration (type-spec integer) (name x) (span (source-span '// &
            '(file main.f90) (start-byte 10) (end-byte 20) (source-hash print-variable-multiply-expression))))) '// &
            ...
            '(source-hash print-variable-multiply-expression))))) '// &
            '(assignment (assignment-stmt (variable x) (expression (assignment-expression '// &
...
end program test_frontend_ast_v2_print_mul
