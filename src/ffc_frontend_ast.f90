...
contains
    ...
    logical function ffc_lower_frontend_ast_v2_from_sx(serialized, body, message) result(lowered)
        ...
        integer(int32) :: initializer_value, addend_value, subtrahend_value, multiplier_value, &
            divisor_value, power_value, route, initialized_xvariable_binary_opcode, print_result_kind
        character(len=11) :: initialized_xvariable_binary_name
        character(len=:), allocatable :: print_result_type, print_source_rule
        ...
        if (assignment_count == 2 .and. trim(assignments(1)%target) == 'x' .and. &
            ...
            initialized_xplus_addend = parse_bounded_binary_expression(trim(assignments(2)%value), &
                '+', bounded_integer_addend_minimum, bounded_integer_addend_maximum, 'addend', addend_value)
            initialized_xminus_subtrahend = parse_bounded_binary_expression(trim(assignments(2)%value), &
                '–', bounded_integer_subtrahend_minimum, bounded_integer_subtrahend_maximum, 'subtrahend', &
                subtrahend_value)
            initialized_xmultiply_multiplier = parse_bounded_binary_expression(trim(assignments(2)%value), &
                '*', bounded_integer_multiplier_minimum, bounded_integer_multiplier_maximum, 'multiplier', &
                multiplier_value)
            initialized_xdivide_divisor = parse_bounded_binary_expression(trim(assignments(2)%value), '/', &
        ...
        if (len_trim(print_statement) > 0) then
            ...
            if (index(print_statement, '( output-count ') /= 0) then
                do assignment_index = 10, 2, -1
                    if (frontend_ast_v2_print_variable_item_count_match(print_statement, assignment_index)) then
                        call emit_frontend_ast_v2_print_variable_count(body, assignment_index, value_kind_integer, &
                            'i32', 'frontend-ast-v2/print-stmt')
                        lowered = ffc_validate_frontend_ast_v2_print_variable_items_shape(body, assignment_index, &
                            int(2 * assignment_index + 7, int32), value_kind_integer, 'i32', &
                            'frontend-ast-v2/print-stmt', message)
                        return
                    end if
                end do
                do assignment_index = 11, 100
                    if (frontend_ast_v2_print_variable_item_count_match(print_statement, assignment_index)) then
                        if (assignment_index <= 60) then
                            print_result_kind = instruction_shape_v2_pow_print_41_60_result_kind
                            print_result_type = instruction_shape_v2_pow_print_41_60_result_type
                            print_source_rule = instruction_shape_v2_pow_print_41_60_source_rule
                        else if (assignment_index <= 80) then
                            print_result_kind = instruction_shape_v2_pow_print_61_80_result_kind
                            print_result_type = instruction_shape_v2_pow_print_61_80_result_type
                            print_source_rule = instruction_shape_v2_pow_print_61_80_source_rule
                        else
                            print_result_kind = instruction_shape_v2_pow_print_81_100_result_kind
                            print_result_type = instruction_shape_v2_pow_print_81_100_result_type
                            print_source_rule = instruction_shape_v2_pow_print_81_100_source_rule
                        end if
                        call emit_frontend_ast_v2_print_variable_count(body, assignment_index, print_result_kind, &
                            print_result_type, print_source_rule)
                        lowered = ffc_validate_frontend_ast_v2_print_variable_items_shape(body, assignment_index, &
                            int(2 * assignment_index + 7, int32), &
                            print_result_kind, print_result_type, print_source_rule, message)
                        return
                    end if
                end do
                if (index(print_statement, '( output-count ') /= 0) then
    ...
    subroutine emit_frontend_ast_v2_print_variable_count(body, item_count, result_kind, result_type, source_rule)
        type(mir_function_body_t), intent(inout) :: body
        integer, intent(in) :: item_count
        ...
        call emit_frontend_ast_v2_print_variable_items(body, item_count, 3, 2, int(size(opcodes), int32), &
            opcodes, result_kind, result_type, source_rule)
    end subroutine emit_frontend_ast_v2_print_variable_count
    ...
    logical function parse_bounded_binary_expression(serialized, operator_token, minimum, maximum, diagnostic_label, &
        ...
        ok = expect_token(token, token_count, position, 'operator', message)
        if (.not. ok) return
        ok = expect_token(token, token_count, position, operator_token, message)
        if (.not. ok) return
        ok = expect_token(token, token_count, position, ')', message)
        if (.not. ok) return
        ok = expect_token(token, token_count, position, '(', message)
        ...
        if (value < minimum .or. value > maximum) then
            call set_message(message, 'unsupported-frontend-ast-v2-integer-'//trim(diagnostic_label))
            ok = .false.
            return
        end if
        ok = expect_token(token, token_count, position, ')', message)
        ...
        if (.not. ok) return
        if (position <= token_count) then
            call set_message(message, 'malformed-frontend-ast-v2-integer-'//trim(diagnostic_label))
            ok = .false.
        end if
    end function parse_bounded_binary_expression
